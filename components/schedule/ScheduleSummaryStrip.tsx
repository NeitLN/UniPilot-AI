...
export function NextClassCard({ block, now }: { block: ClassBlockLite | null; now: Date }) {
  if (!block) {
    return (
      <div className="flex flex-1 items-center gap-3 rounded-card bg-card p-4">
        <IconChip icon={<GraduationCap className="h-[18px] w-[18px]" aria-hidden="true" />} tone="violet" />
        <div>
          <p className="text-[11px] font-bold uppercase tracking-wide text-ink-3">Next class</p>
          <p className="mt-1 text-sm font-bold text-foreground">No more classes today</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-1 items-start gap-3 rounded-card bg-card p-4">
      <IconChip
...
export function ScheduleSummaryStrip({
  ...
  return (
    <div className="flex flex-col gap-2.5 sm:flex-row">
      ...
      <StatTile
        icon={<BookOpen className="h-[18px] w-[18px] text-violet" aria-hidden="true" />}
        label={classesTodayCount === 1 ? "class today" : "classes today"}
        value={String(classesTodayCount)}
        className="flex-1"
      />
      <StatTile
        icon={<CalendarClock className="h-[18px] w-[18px] text-mint-text" aria-hidden="true" />}
