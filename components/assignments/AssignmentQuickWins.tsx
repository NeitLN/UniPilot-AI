...
export function AssignmentQuickWins({ items }: { items: QuickWinItem[] }) {
  return (
    <div className="rounded-card-sm bg-card p-4">
      ...
      <p className="mt-0.5 text-[11.5px] font-semibold text-ink-3">Already mostly done — easy to wrap up.</p>
      <div className="mt-3 flex flex-col gap-1.5">
        {items.map((item) => (
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate text-[12.5px] font-bold text-foreground">{item.title}</span>
            <span className="shrink-0 text-[11px] font-bold text-ink-3">{Math.round(item.progress)}%</span>
