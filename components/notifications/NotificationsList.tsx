    ...
    return (
      <div className="flex flex-col items-center gap-2 rounded-card bg-card py-14 text-center">
        ...
        <p className="text-[12.5px] font-semibold text-ink-3">
          Reminders for assignments, plan sessions, and workload warnings show up here.
        </p>
      </div>
    );
  }
    ...
    <div className="rounded-card bg-card p-4">
      ...
      {unreadCount > 0 && (
        <div className="flex items-center justify-between border-b border-line pb-3">
          <p className="text-[12.5px] font-semibold text-ink-2">{unreadCount} unread</p>
          <button
            type="button"
          ...
          <li key={n.id}>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                {!n.readAt && <span className="mt-0.5 h-2 w-2 shrink-0 rounded-full bg-violet" />}
              </div>
              ...
              {n.body && <p className="mt-0.5 text-[12.5px] font-semibold text-ink-2">{n.body}</p>}
              {n.scheduledAt && (
