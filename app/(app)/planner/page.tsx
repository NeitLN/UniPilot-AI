...
async function loadSessions(
  ...
  const courseIds = [
    ...new Set(
      (assignmentRows ?? []).map((a) => a.course_id).filter((id): id is string => Boolean(id)),
    ),
  ];
...
export default async function PlannerPage() {
  ...
  if (isDraft) {
    ...
    lifecycle =
      computePlanProgress(sessions.map((s) => ({ startAt: s.startAt }))).lifecycle === "ended"
        ? "ended"
        : "active";
  }
          ...
          {lifecycle === "draft" && sessions.length > 0 && (
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-card-sm bg-coral-tint px-4 py-3.5">
              <div className="flex items-start gap-2.5">
                <AlertTriangle
                  className="mt-0.5 h-5 w-5 shrink-0 text-coral-text"
                  aria-hidden="true"
                />
                <div>
                  <p className="text-[12.5px] font-extrabold text-coral-text">
                    Nothing is scheduled until you confirm.
                  </p>
                  <p className="text-[12.5px] font-semibold text-coral-text/85">
                    Review your plan and confirm to lock it in.
                  </p>
                </div>
              </div>
              <a
