...
UniPilot is a _calm, academic_ product, not a game. Every animation here exists to answer one of two questions for the user: **"did that work?"** or **"where did that go/come from?"** — never to decorate.
...
| **Numeric count-up (GPA, streaks, scores)**         | Rejected                                  | The brief itself says "do not make academic performance feel like a casino." All value displays render their final number directly; only the _visual representation_ (bar height/fill) transitions, never the digits.                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
3. **Enhance existing shared components in place rather than building parallel "Animated" versions.** `Modal.tsx`, `FieldError.tsx`, `ProgressBar.tsx`, `Tag.tsx` are all the _same_ components as before, just internally upgraded — every call site kept working. Don't create `AnimatedModal.tsx` next to `Modal.tsx`.
